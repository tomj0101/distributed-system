...
import com.github.tomj0101.ebankv1.ordersystem.service.MailService;
...
import com.github.tomj0101.ebankv1.ordersystem.web.rest.errors.*;
...
public class AccountResource {
    ...
    public void registerAccount(@Valid @RequestBody ManagedUserVM managedUserVM) {
        ...
        UserV1 user = userService.registerUser(managedUserVM, managedUserVM.getPassword());
    ...
    public void activateAccount(@RequestParam(value = "key") String key) {
        Optional<UserV1> user = userService.activateRegistration(key);
        if (!user.isPresent()) {
    ...
    public void saveAccount(@Valid @RequestBody AdminUserDTO userDTO) {
        ...
        Optional<UserV1> existingUser = userRepository.findOneByEmailIgnoreCase(userDTO.getEmail());
        if (existingUser.isPresent() && (!existingUser.get().getLogin().equalsIgnoreCase(userLogin))) {
            throw new EmailAlreadyUsedException();
        }
        Optional<UserV1> user = userRepository.findOneByLogin(userLogin);
        if (!user.isPresent()) {
    ...
    public void requestPasswordReset(@RequestBody String mail) {
        Optional<UserV1> user = userService.requestPasswordReset(mail);
        if (user.isPresent()) {
    ...
    public void finishPasswordReset(@RequestBody KeyAndPasswordVM keyAndPassword) {
        ...
        Optional<UserV1> user = userService.completePasswordReset(keyAndPassword.getNewPassword(), keyAndPassword.getKey());
