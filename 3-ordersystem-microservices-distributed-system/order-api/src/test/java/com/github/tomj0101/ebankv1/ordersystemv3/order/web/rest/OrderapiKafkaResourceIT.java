...
import com.github.tomj0101.ebankv1.ordersystemv3.order.config.KafkaProperties;
...
class OrderapiKafkaResourceIT {
    ...
    void setup() {
        ...
        OrderapiKafkaResource kafkaResource = new OrderapiKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/orderapi-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/orderapi-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
