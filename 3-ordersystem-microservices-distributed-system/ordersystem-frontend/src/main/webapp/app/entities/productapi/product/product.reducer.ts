...
const apiUrl = 'services/productapi/api/products';
const apiSearchUrl = 'services/productapi/api/_search/products';

// Actions

export const getSearchEntities: ICrudSearchAction<IProduct> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_PRODUCTS,
  payload: axios.get<IProduct>(`${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`),
});
