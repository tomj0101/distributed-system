...
describe('OrderMaster e2e test', () => {
  ...
  before(() => {
    ...
    cy.clearCookies();
    cy.intercept('GET', '/services/orderapi/api/order-masters*').as('entitiesRequest');
    cy.visit('');
  ...
  it('should create an instance of OrderMaster', () => {
    ...
    cy.get(`[data-cy="registerDate"]`).type('2021-06-22T11:11').invoke('val').should('equal', '2021-06-22T11:11');

    cy.get(entityCreateSaveButtonSelector).click({ force: true });
    cy.scrollTo('top', { ensureScrollable: false });
    cy.get(entityCreateSaveButtonSelector).should('not.exist');
    cy.intercept('GET', '/services/orderapi/api/order-masters*').as('entitiesRequestAfterCreate');
    cy.visit('/');
    cy.clickOnEntityMenuItem('order-master');
  ...
  it('should delete last instance of OrderMaster', () => {
    cy.intercept('GET', '/services/orderapi/api/order-masters*').as('entitiesRequest');
    cy.intercept('GET', '/services/orderapi/api/order-masters/*').as('dialogDeleteRequest');
    cy.intercept('DELETE', '/services/orderapi/api/order-masters/*').as('deleteEntityRequest');
    cy.visit('/');
    cy.clickOnEntityMenuItem('order-master');
    cy.wait('@entitiesRequest').then(({ request, response }) => {
      ...
      if (startingEntitiesCount > 0) {
        ...
        cy.intercept('GET', '/services/orderapi/api/order-masters*').as('entitiesRequestAfterDelete');
        cy.visit('/');
        cy.clickOnEntityMenuItem('order-master');
