...
describe('Product e2e test', () => {
  ...
  before(() => {
    ...
    cy.clearCookies();
    cy.intercept('GET', '/services/productapi/api/products*').as('entitiesRequest');
    cy.visit('');
  ...
  it('should create an instance of Product', () => {
    ...
    cy.get(`[data-cy="registerDate"]`).type('2021-06-22T21:07').invoke('val').should('equal', '2021-06-22T21:07');

    cy.get(entityCreateSaveButtonSelector).click({ force: true });
    cy.scrollTo('top', { ensureScrollable: false });
    cy.get(entityCreateSaveButtonSelector).should('not.exist');
    cy.intercept('GET', '/services/productapi/api/products*').as('entitiesRequestAfterCreate');
    cy.visit('/');
    cy.clickOnEntityMenuItem('product');
  ...
  it('should delete last instance of Product', () => {
    cy.intercept('GET', '/services/productapi/api/products*').as('entitiesRequest');
    cy.intercept('GET', '/services/productapi/api/products/*').as('dialogDeleteRequest');
    cy.intercept('DELETE', '/services/productapi/api/products/*').as('deleteEntityRequest');
    cy.visit('/');
    cy.clickOnEntityMenuItem('product');
    cy.wait('@entitiesRequest').then(({ request, response }) => {
      ...
      if (startingEntitiesCount > 0) {
        ...
        cy.intercept('GET', '/services/productapi/api/products*').as('entitiesRequestAfterDelete');
        cy.visit('/');
        cy.clickOnEntityMenuItem('product');
