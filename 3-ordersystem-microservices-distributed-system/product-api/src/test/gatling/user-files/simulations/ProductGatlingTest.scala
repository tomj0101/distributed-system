...
class ProductGatlingTest extends Simulation {
    ...
    val baseURL = Option(System.getProperty("baseURL")) getOrElse """http://localhost:8083"""
    ...
    val scn = scenario("Test the Product entity")
        ...
        .repeat(2) {
            ...
            .get("/services/productapi/api/products")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new product")
            .post("/services/productapi/api/products")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .repeat(5) {
                ...
                .get("/services/productapi${new_product_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created product")
            .delete("/services/productapi${new_product_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
